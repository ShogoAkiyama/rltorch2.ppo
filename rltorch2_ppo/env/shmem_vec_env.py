...
class ShmemVecEnv(VecEnv):
    ...
    def __init__(self, env_fns, context='spawn'):
        ...
        self.obs_keys, self.obs_shapes, \
            self.obs_dtypes = obs_space_info(observation_space)
        self.obs_bufs = [{
            k: ctx.Array(
                _NP_TO_CT[self.obs_dtypes[k].type],
                int(np.prod(self.obs_shapes[k]))
                ) for k in self.obs_keys} for _ in env_fns]
        self.parent_pipes = []
        self.procs = []
        with clear_mpi_env_vars():
            for env_fn, obs_buf in zip(env_fns, self.obs_bufs):
                ...
                proc = ctx.Process(
                    ...
                    args=(
                        child_pipe, parent_pipe, wrapped_fn, obs_buf,
    ...
    def _decode_obses(self, obs):
        ...
        for k in self.obs_keys:
            ...
            o = [np.frombuffer(
                b.get_obj(),
                dtype=self.obs_dtypes[k]
            ).reshape(self.obs_shapes[k]) for b in bufs]
            result[k] = np.array(o)
        return dict_to_obs(result)


def _subproc_worker(pipe, parent_pipe, env_fn_wrapper, obs_bufs, obs_shapes,
                    obs_dtypes, keys):
    """
